...
CREATE TABLE IF NOT EXISTS forecast_runs (
    ...
    target_property_type_id VARCHAR,       -- NULL = no property-type dim; maps to 'all'
...
WITH joined AS (
    ...
    JOIN fact_timeseries f
     ...
     AND f.property_type_id = COALESCE(r.target_property_type_id, 'all')
...
SELECT
    ...
    MAX(CASE WHEN horizon_months = 3  THEN mae END) AS mae_3m,
    MAX(CASE WHEN horizon_months = 6  THEN mae END) AS mae_6m,
    MAX(CASE WHEN horizon_months = 12 THEN mae END) AS mae_12m,

    MAX(CASE WHEN horizon_months = 1  THEN rmse END) AS rmse_1m,
    MAX(CASE WHEN horizon_months = 3  THEN rmse END) AS rmse_3m,
    MAX(CASE WHEN horizon_months = 6  THEN rmse END) AS rmse_6m,
    MAX(CASE WHEN horizon_months = 12 THEN rmse END) AS rmse_12m
FROM per_horizon
GROUP BY run_id;


-- 4) Evaluation view — LONG FORM, PER-PREDICTION ROWS (correct grain)
...
SELECT
    ...
    (f.value - p.y_hat) AS err,
    ABS(f.value - p.y_hat) AS abs_err,
    POWER(f.value - p.y_hat, 2) AS sq_err,
    (ABS((f.value - p.y_hat) / NULLIF(f.value, 0)) * 100.0) AS ape
FROM forecast_predictions p
