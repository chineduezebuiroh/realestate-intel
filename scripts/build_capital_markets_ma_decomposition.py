...
def _metric_weight_evidence(settled_scores, frames, registry):
    ...
    if not np.array_equal(pivot.iloc[:,0].to_numpy(),pivot.iloc[:,1].to_numpy(),equal_nan=True): raise ValueError("Metric-score chronology differs across metric-weight policies")
    ...
    recent=dimensions.pivot(index="date",columns="policy",values="dimension_score").tail(36).reset_index()
    for policy in METRIC_WEIGHT_POLICIES:
        recent[f"{policy} monthly movement"]=recent[policy].diff(); c=contributions.query("policy==@policy").pivot(index="date",columns="metric",values="weighted_contribution").tail(36); recent[f"{policy} Fed Funds contribution"]=recent.date.map(c.fedfunds); recent[f"{policy} long-rate-family contribution"]=recent.date.map(c[list(METRIC_WEIGHT_FAMILIES['long_rate_family'])].sum(axis=1)); recent[f"{policy} spread-family contribution"]=recent.date.map(c[list(METRIC_WEIGHT_FAMILIES['spread_family'])].sum(axis=1))
...
def main() -> None:
    ...
    mw_review=f"<section><h1>Capital Markets Metric-Weight Review</h1><p><strong>Fed Funds rises from 15% to one-third under MW-EQUAL-FAMILY.</strong> Similarity to incumbent is not a success criterion. Lower concentration is not automatically better. The challenger must improve or preserve regime-quality behavior without allowing Fed Funds noise to dominate the dimension.</p><h2>1. Exact policies and equal-family decomposition</h2>{tables['capital_markets_metric_weight_policy_registry'].to_html(index=False,border=0)}<h2>2. Decision matrix</h2>{tables['capital_markets_metric_weight_decision_matrix'].to_html(index=False,border=0,classes='decision-matrix')}<h2>3. Contribution and concentration</h2>{tables['capital_markets_metric_weight_concentration_summary'].to_html(index=False,border=0)}{tables['capital_markets_metric_weight_family_contribution_summary'].to_html(index=False,border=0)}<h2>4. Fed Funds stress</h2>{tables['capital_markets_metric_weight_fedfunds_stress'].to_html(index=False,border=0)}<h2>5. Extreme-jump attribution</h2>{tables['capital_markets_metric_weight_extreme_jumps'].to_html(index=False,border=0)}<h2>6. Dimension stability and turning points</h2>{tables['capital_markets_metric_weight_dimension_stability'].to_html(index=False,border=0)}{tables['capital_markets_metric_weight_dimension_turning_point_summary'].to_html(index=False,border=0)}<h2>7. Recent 36-month chronology</h2>{tables['capital_markets_metric_weight_recent_chronology'].to_html(index=False,border=0)}<h2>8. Downstream context</h2><a href='evidence/capital_markets_metric_weight_axis_propagation.csv'>Axis propagation CSV</a> · <a href='evidence/capital_markets_metric_weight_regime_change_summary.csv'>Regime changes CSV</a></section>"
