...
TABLES = (
    ...
    "capital_markets_spread_correction_cancellation_audit",
    "capital_markets_spread_pathology_reconciliation",
    ...
    "capital_markets_final_feature_weight_policy_registry",
    "capital_markets_final_feature_weight_metric_stability",
    "capital_markets_final_feature_weight_metric_turning_point_summary",
    "capital_markets_final_feature_weight_family_summary",
    "capital_markets_final_feature_weight_dimension_chronology",
    "capital_markets_final_feature_weight_dimension_stability",
    "capital_markets_final_feature_weight_dimension_turning_point_summary",
    "capital_markets_final_feature_weight_extreme_jumps",
    "capital_markets_final_feature_weight_cancellation",
    "capital_markets_final_feature_weight_recent_chronology",
    "capital_markets_final_feature_weight_directional_context",
    "capital_markets_final_feature_weight_regime_change_summary",
    "capital_markets_final_feature_weight_decision_matrix",
    "capital_markets_final_feature_weight_human_decision_status",
    "capital_markets_final_feature_weight_runtime_summary",
    "capital_markets_metric_weight_policy_registry", "capital_markets_metric_weight_metric_chronology",
    "capital_markets_metric_weight_metric_stability", "capital_markets_metric_weight_metric_turning_point_summary",
    "capital_markets_metric_weight_family_contribution_summary", "capital_markets_metric_weight_contribution_chronology",
    "capital_markets_metric_weight_concentration_summary", "capital_markets_metric_weight_dimension_chronology",
    "capital_markets_metric_weight_dimension_stability", "capital_markets_metric_weight_dimension_turning_point_summary",
    "capital_markets_metric_weight_extreme_jumps", "capital_markets_metric_weight_cancellation",
    "capital_markets_metric_weight_recent_chronology", "capital_markets_metric_weight_directional_context",
    "capital_markets_metric_weight_axis_propagation", "capital_markets_metric_weight_regime_change_summary",
    "capital_markets_metric_weight_parity_audit", "capital_markets_metric_weight_decision_matrix",
    "capital_markets_metric_weight_human_decision_status", "capital_markets_metric_weight_runtime_summary",
    "capital_markets_metric_weight_fedfunds_stress",
...
def _spread_correction_evidence(registry, active, caches, national_metrics, frames, corrected_raw, governed_pathology):
    ...
    pathology_reconciliation = reconcile_spread_pathology(governed_pathology)
    governed_pathology_by_metric = pathology_reconciliation.groupby("metric_key", sort=True).agg(
        zero_crossing_count=("zero_crossing_count", "sum"),
        ratio_near_zero_denominator_count=("near_zero_denominator_count", "sum"),
        ratio_direction_conflict_count=("direction_conflict_count", "sum"),
        ratio_non_finite_count=("non_finite_ratio_count", "sum"),
    )
    ...
    for policy in policies:
        ...
        contributions=contributions.sort_values(["canonical_metric_key","date"],kind="mergesort")
        contributions["prior_date"]=contributions.groupby("canonical_metric_key").date.shift()
        contributions["movement"]=contributions.groupby("canonical_metric_key").contribution.diff()
        contiguous=(contributions.date.dt.to_period("M")-contributions.prior_date.dt.to_period("M")).map(
            lambda value: value.n == 1 if pd.notna(value) else False)
        contributions.loc[~contiguous,"movement"]=np.nan
        parent_frame=dim.sort_values("date",kind="mergesort").copy()
        parent_frame["prior_date"]=parent_frame.date.shift()
        parent_frame["parent_movement"]=parent_frame.dimension_score.diff()
        parent_contiguous=(parent_frame.date.dt.to_period("M")-parent_frame.prior_date.dt.to_period("M")).map(
            lambda value: value.n == 1 if pd.notna(value) else False)
        parent_frame.loc[~parent_contiguous,"parent_movement"]=np.nan
        parent=parent_frame.set_index("date").parent_movement
        for date,g in contributions.groupby("date"):
            count=int(g.metric_score.notna().sum()); weight_sum=float(g.configured_weight.sum())
            all_six=count==6 and set(g.canonical_metric_key)==set(active)
            all_child=all_six and g.movement.notna().all(); pm=parent.get(date,np.nan)
            parent_valid=pd.notna(pm)
            comparable=bool(all_child and parent_valid)
            child_sum=g.movement.sum(min_count=6); total=g.movement.abs().sum(min_count=6)
            raw=total-abs(pm) if comparable else np.nan
            reconstruction=child_sum-pm if comparable else np.nan
            if comparable and abs(reconstruction)>CANCELLATION_TOLERANCE:
                raise ValueError(f"Cancellation child reconstruction failed: {policy} {date}")
            if comparable and raw < -CANCELLATION_TOLERANCE:
                raise ValueError(f"Materially negative comparable cancellation: {policy} {date}")
            governed=0.0 if comparable and raw < 0 else raw
            if comparable: reason=""
            elif not parent_valid and g.prior_date.isna().all(): reason="leading_feature_warmup"
            elif not all_six: reason="incomplete_metric_availability"
            elif not all_child: reason="child_movement_unavailable"
            else: reason="non_contiguous_prior_month"
            cancellation_rows.append({"policy":policy,"date":date,"available_metric_count":count,
                "available_weight_sum":weight_sum,"all_six_metrics_available":all_six,
                "parent_movement":pm,"sum_absolute_child_movement":total,
                "absolute_parent_movement":abs(pm) if parent_valid else np.nan,
                "cancellation_amount_raw":raw,"cancellation_amount_governed":governed,
                "cancellation_ratio":governed/total if comparable and total > 0 else np.nan,
                "comparable_cancellation_flag":comparable,"exclusion_reason":reason,
        ...
        for metric in spreads:
            ...
            if policy == policies[0]:
                governed = governed_pathology_by_metric.loc[metric].to_dict()
                ms.update(governed)
            else:
    ...
    cancellation=pd.DataFrame(cancellation_rows)
    comparable_cancellation=cancellation.loc[cancellation.comparable_cancellation_flag]
    cancels=comparable_cancellation.groupby("policy").cancellation_ratio.agg(median="median",p90=lambda s:s.quantile(.9),p99=lambda s:s.quantile(.99))
...
def _metric_weight_evidence(settled_scores, frames, registry):
    ...
    architecture="mortgage_30y:ratio_ma12|mortgage_15y:ratio_ma12|treasury_10y:ratio_ma12|fedfunds:ratio_ma3|spread_10y_2y:arithmetic_difference_ma9|spread_10y_fedfunds:arithmetic_difference_ma9"
    for policy,weights in METRIC_WEIGHT_POLICIES.items():
        for metric,weight in weights.items():
            registry_rows.append({"policy":policy,"metric":metric,"family":family_of[metric],"configured_metric_weight":weight,"configured_family_weight":sum(weights[m] for m in METRIC_WEIGHT_FAMILIES[family_of[metric]]),"feature_weights":"0.60|0.20|0.20","architecture":architecture,"only_policy_difference":"metric_weights"})
        work=scores.copy(); work["metric"]=work.canonical_metric_key; work["family"]=work.metric.map(family_of); work["policy"]=policy
        work["configured_metric_weight"]=work.metric.map(weights); work["availability_flag"]=work.metric_score.notna()
        work["available_weight_sum"]=work.configured_metric_weight.where(work.availability_flag,0).groupby(work.date).transform("sum")
        work["effective_metric_weight"]=work.configured_metric_weight.where(work.availability_flag,0)/work.available_weight_sum
        work["weighted_contribution"]=work.metric_score*work.effective_metric_weight
        contribution_rows.append(work[["date","policy","metric","family","metric_score","configured_metric_weight","effective_metric_weight","weighted_contribution","availability_flag","available_weight_sum"]])
        metric_rows.append(work[["date","policy","metric","family","metric_score"]])
    contributions=pd.concat(contribution_rows,ignore_index=True); metric_chron=pd.concat(metric_rows,ignore_index=True)
    dimensions=contributions.groupby(["policy","date"],as_index=False).weighted_contribution.sum(min_count=1).rename(columns={"weighted_contribution":"dimension_score"})
    recon=contributions.groupby(["policy","date"],as_index=False).weighted_contribution.sum(min_count=1).merge(dimensions,on=["policy","date"],validate="one_to_one")
    if not np.allclose(recon.weighted_contribution,recon.dimension_score,atol=CANCELLATION_TOLERANCE,rtol=0,equal_nan=True): raise ValueError("Metric contributions do not reconstruct Capital Markets")
    pivot=metric_chron.pivot(index=["date","metric"],columns="policy",values="metric_score")
    incumbent_scores=pivot["MW-INCUMBENT"].to_numpy()
    if any(not np.array_equal(incumbent_scores,pivot[p].to_numpy(),equal_nan=True) for p in pivot.columns): raise ValueError("Metric-score chronology differs across metric-weight policies")
    stability=pd.DataFrame([_stability(g,"dimension_score",policy=p) for p,g in dimensions.groupby("policy",sort=False)])
    metric_stability=pd.DataFrame([{**_stability(g,"metric_score"),"policy":p,"metric":m,"family":family_of[m]} for (p,m),g in metric_chron.groupby(["policy","metric"],sort=False)])
    turns=[]; turn_summary=[]; attributed=[]; latest=dimensions.date.max()
    contributions["contribution_movement"]=contributions.groupby(["policy","metric"]).weighted_contribution.diff()
    for policy,g in dimensions.groupby("policy",sort=False):
        tp=detect_turning_points(g,"dimension_score"); tp["policy"]=policy; turns.append(tp)
        q=tp.loc[tp.qualified].sort_values("turning_point_date"); spacing=q.turning_point_date.diff().dt.days/30.4375
        turn_summary.append({"policy":policy,"turning_points":len(q),"median_turn_spacing_months":spacing.median(),"latest_36m_turning_points":int(q.turning_point_date.gt(latest-pd.DateOffset(months=36)).sum())})
        for t in q.itertuples(index=False):
            drivers=contributions.loc[contributions.policy.eq(policy)&contributions.date.eq(t.turning_point_date)].dropna(subset=["contribution_movement"])
            driver=drivers.loc[drivers.contribution_movement.abs().idxmax()] if len(drivers) else None
            attributed.append({"policy":policy,"turning_point_date":t.turning_point_date,"turning_point_type":t.turning_point_type,"largest_driver_metric":driver.metric if driver is not None else pd.NA,"largest_driver_family":driver.family if driver is not None else pd.NA,"fedfunds_driver_flag":bool(driver is not None and driver.metric=="fedfunds"),"attribution_scope":"descriptive_not_causal"})
    turn_summary=pd.DataFrame(turn_summary); turning_attribution=pd.DataFrame(attributed)
    driver_shares=turning_attribution.groupby("policy").fedfunds_driver_flag.mean()
    turn_summary["fedfunds_driver_share"]=turn_summary.policy.map(driver_shares)
    movement=dimensions.copy(); movement["dimension_movement"]=movement.groupby("policy").dimension_score.diff(); movement["absolute_dimension_movement"]=movement.dimension_movement.abs()
    extreme=[]
    ...
    extreme["top_10_count_long_rate_driver"]=extreme.policy.map(driver_counts.get("long_rate_family",pd.Series(dtype=int)))
    extreme["top_10_count_spread_driver"]=extreme.policy.map(driver_counts.get("spread_family",pd.Series(dtype=int)))
    cancel=[]
    for (policy,date),g in contributions.groupby(["policy","date"],sort=True):
        total=g.contribution_movement.abs().sum(min_count=1); parent=movement.query("policy==@policy and date==@date").absolute_dimension_movement.iloc[0]; comparable=pd.notna(total) and total>0 and pd.notna(parent)
        cancel.append({"policy":policy,"date":date,"comparable":comparable,"cancellation_ratio":1-parent/total if comparable else np.nan,"total_absolute_child_movement":total,"absolute_parent_movement":parent})
    cancellation=pd.DataFrame(cancel); cancel_summary=cancellation.query("comparable").groupby("policy").cancellation_ratio.agg(median_cancellation="median",p90_cancellation=lambda s:s.quantile(.9),p99_cancellation=lambda s:s.quantile(.99)).reset_index()
    abs_c=contributions.weighted_contribution.abs(); contributions["absolute_contribution_share"]=abs_c/abs_c.groupby([contributions.policy,contributions.date]).transform("sum")
    monthly=contributions.groupby(["policy","date"]).agg(largest_effective_metric_weight=("effective_metric_weight","max"),largest_absolute_contribution_share=("absolute_contribution_share","max"),contribution_hhi=("absolute_contribution_share",lambda s:(s*s).sum())).reset_index()
    concentration=[]; family_summary=[]
    for policy,w in METRIC_WEIGHT_POLICIES.items():
        mo=monthly.query("policy==@policy"); concentration.append({"policy":policy,"largest_configured_metric_weight":max(w.values()),"largest_effective_metric_weight_median":mo.largest_effective_metric_weight.median(),"largest_absolute_contribution_share_median":mo.largest_absolute_contribution_share.median(),"largest_absolute_contribution_share_p90":mo.largest_absolute_contribution_share.quantile(.9),"configured_weight_hhi":sum(x*x for x in w.values()),"median_monthly_contribution_hhi":mo.contribution_hhi.median(),"p90_monthly_contribution_hhi":mo.contribution_hhi.quantile(.9)})
        for family,members in METRIC_WEIGHT_FAMILIES.items():
            f=contributions.query("policy==@policy and metric in @members").groupby("date").agg(effective_family_weight=("effective_metric_weight","sum"),absolute_family_contribution=("weighted_contribution",lambda s:s.sum())).reset_index(); totals=contributions.query("policy==@policy").groupby("date").weighted_contribution.apply(lambda s:s.abs().sum()); f["absolute_family_contribution_share"]=f.absolute_family_contribution.abs()/f.date.map(totals)
            family_summary.append({"policy":policy,"family":family,"configured_family_weight":sum(w[m] for m in members),"median_effective_family_weight":f.effective_family_weight.median(),"median_absolute_family_contribution_share":f.absolute_family_contribution_share.median(),"p90_absolute_family_contribution_share":f.absolute_family_contribution_share.quantile(.9)})
    concentration=pd.DataFrame(concentration); family_summary=pd.DataFrame(family_summary)
    fed=[]
    for policy,w in METRIC_WEIGHT_POLICIES.items():
        f=contributions.query("policy==@policy and metric=='fedfunds'"); ex=extreme.query("policy==@policy"); ta=turning_attribution.query("policy==@policy"); recent=f.nlargest(36,"date")
        fed.append({"policy":policy,"configured_fedfunds_weight":w["fedfunds"],"median_effective_fedfunds_weight":f.effective_metric_weight.median(),"median_abs_fedfunds_contribution":f.weighted_contribution.abs().median(),"P90_abs_fedfunds_contribution":f.weighted_contribution.abs().quantile(.9),"P99_abs_fedfunds_contribution":f.weighted_contribution.abs().quantile(.99),"max_abs_fedfunds_contribution":f.weighted_contribution.abs().max(),"share_of_top_10_dimension_jumps_with_fedfunds_as_largest_driver":ex.largest_driver_metric.eq("fedfunds").mean(),"share_of_dimension_turning_points_with_fedfunds_as_largest_abs_driver":ta.fedfunds_driver_flag.mean(),"recent_36m_median_abs_fedfunds_contribution":recent.weighted_contribution.abs().median(),"recent_36m_P90_abs_fedfunds_contribution":recent.weighted_contribution.abs().quantile(.9)})
    fed=pd.DataFrame(fed)
    stability_by_policy=stability.set_index("policy")
    correlations={
        "fedfunds_weight_vs_dimension_p90":fed.configured_fedfunds_weight.corr(fed.policy.map(stability_by_policy.p90_absolute_mom_change)),
        "fedfunds_weight_vs_dimension_p99":fed.configured_fedfunds_weight.corr(fed.policy.map(stability_by_policy.p99_absolute_mom_change)),
        "fedfunds_weight_vs_sign_flips":fed.configured_fedfunds_weight.corr(fed.policy.map(stability_by_policy.sign_flip_count)),
        "fedfunds_weight_vs_turning_points":fed.configured_fedfunds_weight.corr(fed.policy.map(turn_summary.set_index("policy").turning_points)),
    }
    for column,value in correlations.items(): fed[column]=value
    downstream={}; axis_rows=[]; regime_summary=[]
    for policy,g in dimensions.groupby("policy",sort=False):
        national=g.assign(geo_id=NATIVE_GEOGRAPHY,dimension="capital_markets")[["geo_id","date","dimension","dimension_score"]]; downstream[policy]=_recompute_governed_descendants(national,frames["dimension_scores"])
    base=downstream["MW-INCUMBENT"]
    for policy,(axes,coords,regs,_,_) in downstream.items():
        ba,bc,br,_,_=base; a=axes.merge(ba[["geo_id","date","axis","axis_score"]],on=["geo_id","date","axis"],suffixes=("","_incumbent")); a["policy"]=policy; a["axis_displacement"]=a.axis_score-a.axis_score_incumbent; axis_rows.append(a)
        keys=[k for k in ("major_regime","minor_regime","regime") if k in regs and k in br]; rr=regs.merge(br[["geo_id","date",*keys]],on=["geo_id","date"],suffixes=("","_incumbent")); changed=pd.Series(False,index=rr.index)
        for k in keys: changed |= rr[k].astype(str).ne(rr[f"{k}_incumbent"].astype(str))
        regime_summary.append({"policy":policy,"changed_county_month_regimes":int(changed.sum()),"latest_36m_changed_regimes":int((changed&rr.date.gt(rr.date.max()-pd.DateOffset(months=36))).sum()),"changed_share":changed.mean()})
    axes=pd.concat(axis_rows,ignore_index=True); regime_summary=pd.DataFrame(regime_summary)
    recent_rows=[]
    for policy,g in dimensions.groupby("policy",sort=False):
        d=g.copy(); d["monthly_movement"]=d.dimension_score.diff(); d=d.tail(36).copy()
        c=contributions.query("policy==@policy").pivot(index="date",columns="metric",values="weighted_contribution")
        d["fedfunds_contribution"]=d.date.map(c.fedfunds)
        d["long_rate_family_contribution"]=d.date.map(c[list(METRIC_WEIGHT_FAMILIES["long_rate_family"])].sum(axis=1))
        d["spread_family_contribution"]=d.date.map(c[list(METRIC_WEIGHT_FAMILIES["spread_family"])].sum(axis=1))
        recent_rows.append(d[["policy","date","dimension_score","monthly_movement","fedfunds_contribution","long_rate_family_contribution","spread_family_contribution"]])
    recent=pd.concat(recent_rows,ignore_index=True)
    directional=pd.DataFrame([{"policy":p,**directional_agreement(dimensions.query("policy=='MW-INCUMBENT'"),dimensions.query("policy==@p"),"dimension_score",h)} for p in METRIC_WEIGHT_POLICIES for h in (1,3,6,12)])
    matrix=[]
    for policy,w in METRIC_WEIGHT_POLICIES.items():
        co=concentration.query("policy==@policy").iloc[0]; st=stability.query("policy==@policy").iloc[0]; ts=turn_summary.query("policy==@policy").iloc[0]; ca=cancel_summary.query("policy==@policy").iloc[0]; fs=fed.query("policy==@policy").iloc[0]; rg=regime_summary.query("policy==@policy").iloc[0]
        family_definition="|".join(f"{family}={sum(w[m] for m in members)}" for family,members in METRIC_WEIGHT_FAMILIES.items())
        matrix.append({"Policy":policy,"Family-weight definition":family_definition,"Metric-weight definition":"|".join(f"{m}={v}" for m,v in w.items()),"Largest configured metric weight":co.largest_configured_metric_weight,"Configured-weight HHI":co.configured_weight_hhi,"Median contribution HHI":co.median_monthly_contribution_hhi,"P90 contribution HHI":co.p90_monthly_contribution_hhi,"Dimension median abs MoM":st.median_absolute_mom_change,"Dimension P90":st.p90_absolute_mom_change,"Dimension P99":st.p99_absolute_mom_change,"Dimension max jump":st.maximum_absolute_jump,"Dimension rolling-12m volatility median":st.rolling_12m_volatility_median,"Dimension sign flips":st.sign_flip_count,"Dimension turning points":ts.turning_points,"Median turn spacing":ts.median_turn_spacing_months,"Latest-36m turning points":ts.latest_36m_turning_points,"Median cancellation":ca.median_cancellation,"P90 cancellation":ca.p90_cancellation,"P99 cancellation":ca.p99_cancellation,"Fed Funds configured weight":w["fedfunds"],"Fed Funds median abs contribution":fs.median_abs_fedfunds_contribution,"Fed Funds P90 abs contribution":fs.P90_abs_fedfunds_contribution,"Fed Funds share of top-10 jumps as largest driver":fs.share_of_top_10_dimension_jumps_with_fedfunds_as_largest_driver,"Fed Funds share of turns as largest driver":fs.share_of_dimension_turning_points_with_fedfunds_as_largest_abs_driver,"Changed county-month regimes":rg.changed_county_month_regimes,"Latest-36m changed regimes":rg.latest_36m_changed_regimes,"Decision":"pending"})
    status=pd.DataFrame([{"recommendation_state":"none","promotion_state":"none","human_decision":"pending","diagnostic_only":True}])
    parity=pd.DataFrame([{"policy":p,"metric_score_exact_parity":True,"only_metric_weights_differ":True,"feature_weights_60_20_20":True,"settled_mixed_transform_architecture":True,"contribution_reconstruction":True,"supply_unchanged":True,"affordability_unchanged":True,"production_registry_mutated":False} for p in METRIC_WEIGHT_POLICIES])
...
def _feature_weight_evidence(proof, registry, active, caches, national_metrics, native_dims, frames):
    ...
    for policy, mix in policies.items():
        for metric in active:
            ...
            incumbent=False; window, transform_family=corrected_architecture(metric)
            operator="/" if transform_family=="ratio" else "-"
            suffix=" - 1" if transform_family=="ratio" else ""
            definitions={"level":f"MA{window}(raw)","short_term_change":f"MA{window}(raw) {operator} lag3(MA{window}(raw)){suffix}","long_term_change":f"MA{window}(raw) {operator} lag12(MA{window}(raw)){suffix}"}
            actual=mix.get(metric, mix.get("default")) if "default" in mix else mix
            registry_rows.append({"contract_identity":CONTRACT_IDENTITY,"source_run_id":proof.run_id,"experiment_id":proof.experiment_id,
                "policy_id":policy,"policy_label":policy,
                "policy_status":"finalist",
                "metric":metric,"family":families[metric],"transform_family":transform_family,"ma_window":window,
                "level_definition":definitions["level"],"short_definition":definitions["short_term_change"],"long_definition":definitions["long_term_change"],
    ...
    if len(policy_registry)!=18 or not np.isclose(policy_registry[["level_weight","short_weight","long_weight"]].sum(axis=1),1).all():
    ...
    scores={}; dimensions={}; downstream={}
    cache_keys={(m,*corrected_architecture(m)) for m in active}
    ...
    for number,(policy,mix) in enumerate(SETTLED_FEATURE_WEIGHT_POLICIES.items(),1):
        ...
        replacements={}
        for metric in active:
            window, transform_family = corrected_architecture(metric)
            key=(metric,window,transform_family); caches[key]["uses"].append("feature_weight")
            metric_mix=mix.get(metric, mix.get("default")) if "default" in mix else mix
            replacements[metric]=score_metrics_with_feature_weights(caches[key]["normalized"],registry,metric_mix)
    ...
    for policy,frame in dimensions.items():
        ...
        for basis,basis_frame,context in (("FW-A",control,"feature_weight_context"),):
    ...
    for (policy,metric),part in metric_chron.groupby(["policy","metric"],sort=True):
        st=_stability(part,"metric_score"); ct=detect_turning_points(metric_chron.query("policy=='FW-A' and metric==@metric"),"metric_score"); pt=detect_turning_points(part,"metric_score"); mt=match_turning_points(ct,pt); delay=mt.loc[mt.matched,"signed_delay_months"]
        qualified=pt.loc[pt.qualified].sort_values("turning_point_date") if len(pt) else pt
        spacing=qualified.turning_point_date.diff().dt.days.div(30.4375)
        row={"policy":policy,"metric":metric,"family":families[metric],**st,"turning_point_count":int(pt.qualified.sum()) if len(pt) else 0,"median_turn_spacing_months":spacing.median(),"median_turn_prominence":qualified.prominence.median() if len(qualified) else np.nan,"matched_control_turns":int(mt.matched.sum()),"median_turn_delay":delay.abs().median(),"max_turn_delay":delay.abs().max(),"warmup_coverage_parity":len(part)==len(metric_chron.query("policy=='FW-A' and metric==@metric"))}
        for h in (1,3,6,12): row[f"directional_agreement_{h}m_vs_40_30_30"]=directional_agreement(metric_chron.query("policy=='FW-A' and metric==@metric"),part,"metric_score",h)["agreement_share"]
    ...
    if not np.allclose(recon.weighted_contribution,recon.dimension_score,atol=CANCELLATION_TOLERANCE,rtol=0): raise ValueError("Feature-weight contributions do not reconstruct parent")
    ...
    for (policy,date),g in movement.groupby(["policy","date"],sort=True):
        ...
        comparable=pd.notna(pm) and pd.notna(total) and total > 0
        cancel.append({"policy":policy,"date":date,"comparable":comparable,"exclusion_reason":pd.NA if comparable else "warmup_or_zero_child_movement","total_absolute_child_movement":total,"absolute_parent_movement":pm,"parent_child_movement_ratio":pm/total if comparable else np.nan,"cancellation_amount":total-pm if comparable else np.nan,"cancellation_ratio":1-pm/total if comparable else np.nan,"dominant_metric":g.loc[g.child_move.abs().idxmax(),"metric"] if g.child_move.notna().any() else pd.NA,"dominant_family":fam.abs().idxmax() if fam.notna().any() else pd.NA,"offsetting_child_count":int((g.child_move*np.sign(g.child_move.sum())<0).sum()),"metric_sign_alignment":g.child_move.dropna().map(np.sign).nunique()<=1,"family_sign_alignment":fam.dropna().map(np.sign).nunique()<=1})
    cancellation=pd.DataFrame(cancel); comparable=cancellation.loc[cancellation.comparable]
    contribution_summary=comparable.groupby("policy",as_index=False).agg(median_cancellation=("cancellation_ratio","median"),p90_cancellation=("cancellation_ratio",lambda s:s.quantile(.9)),p99_cancellation=("cancellation_ratio",lambda s:s.quantile(.99)),parent_child_movement_ratio=("parent_child_movement_ratio","median"))
    ...
    control_down=downstream["FW-A"]; axis_rows=[]; coord_rows=[]; regime_rows=[]; regime_summary=[]
    control_axes,control_coords,control_reg,_,_=control_down
    for policy in SETTLED_FEATURE_WEIGHT_POLICIES:
        axes,coords,regs,_,_=downstream[policy]; ax=axes.merge(control_axes[["geo_id","date","axis","axis_score"]],on=["geo_id","date","axis"],suffixes=("","_control"),validate="one_to_one"); ax["policy"]=policy; ax["delta_vs_fw_a"]=ax.axis_score-ax.axis_score_control; ax["absolute_delta"]=ax.delta_vs_fw_a.abs(); axis_rows.append(ax)
    ...
    for policy,mix in policies.items():
        st=stab.query("policy==@policy").iloc[0]; dr=dirs.query("policy==@policy and comparison_policy=='FW-A'").set_index("horizon_months").agreement_share; tr=ts.query("policy==@policy").iloc[0]; ca=contribution_summary.query("policy==@policy").iloc[0]; rg=regime_summary.query("policy==@policy")
        weights=policy_registry.query("policy_id==@policy").iloc[0]
        matrix.append({"Policy":policy,"Level weight":weights.level_weight,"Short weight":weights.short_weight,"Long weight":weights.long_weight,"Median abs MoM Δ":st.median_absolute_mom_change,"P90 Δ":st.p90_absolute_mom_change,"P99 Δ":st.p99_absolute_mom_change,"Sign flips":st.sign_flip_count,"Rolling 12m vol":st.rolling_12m_volatility_median,**{f"Direction agree {h}m vs FW-A":dr[h] for h in (1,3,6,12)},"Turning points":tr.challenger_turning_point_count,"Median turn delay":tr.median_absolute_delay,"Max turn delay":tr.maximum_absolute_delay,"Median cancellation":ca.median_cancellation,"P90 cancellation":ca.p90_cancellation,"Demand-axis median abs Δ":0 if policy=="production_incumbent" else axis_summary.loc[policy,"demand"],"Supply-axis median abs Δ":0 if policy=="production_incumbent" else axis_summary.loc[policy,"supply"],"Regime-change share vs FW-A":0 if rg.empty else rg.changed_share.iloc[0],"Latest-36m regime changes":0 if rg.empty else rg.latest_36m_changed_count.iloc[0],"Warmup":len(native_dims)-len(common)})
    ...
    family_summary=metric_summary.groupby(["policy","family"],as_index=False).median(numeric_only=True)
    qualified_turns=turns.loc[turns.qualified].sort_values(["policy","turning_point_date"])
    dimension_turn_summary=[]
    latest=chronology.date.max()
    for policy, group in qualified_turns.groupby("policy",sort=True):
        spacing=group.turning_point_date.diff().dt.days.div(30.4375)
        dimension_turn_summary.append({"policy":policy,"turning_points":len(group),"median_turn_spacing_months":spacing.median(),"median_prominence":group.prominence.median(),"latest_36m_dimension_turns":int(group.turning_point_date.gt(latest-pd.DateOffset(months=36)).sum())})
    dimension_turn_summary=pd.DataFrame(dimension_turn_summary)
    architecture="mortgage_30y:ratio_ma12|mortgage_15y:ratio_ma12|treasury_10y:ratio_ma12|fedfunds:ratio_ma3|spread_10y_2y:arithmetic_difference_ma9|spread_10y_fedfunds:arithmetic_difference_ma9"
    final_matrix=[]
    for policy in policies:
        ms=metric_summary.loc[metric_summary.policy.eq(policy)]; ds=stab.loc[stab.policy.eq(policy)].iloc[0]; dt=dimension_turn_summary.loc[dimension_turn_summary.policy.eq(policy)].iloc[0]; ca=contribution_summary.loc[contribution_summary.policy.eq(policy)].iloc[0]
        final_matrix.append({"Policy":policy,"Architecture":architecture,"Median metric abs MoM":ms.median_absolute_mom_change.median(),"Median metric P90":ms.p90_absolute_mom_change.median(),"Median metric P99":ms.p99_absolute_mom_change.median(),"Total metric sign flips":int(ms.sign_flip_count.sum()),"Total metric turning points":int(ms.turning_point_count.sum()),"Median metric turn spacing":ms.median_turn_spacing_months.median(),"Max metric jump":ms.maximum_absolute_jump.max(),"Dimension median abs MoM":ds.median_absolute_mom_change,"Dimension P90":ds.p90_absolute_mom_change,"Dimension P99":ds.p99_absolute_mom_change,"Dimension max jump":ds.maximum_absolute_jump,"Dimension sign flips":ds.sign_flip_count,"Dimension turning points":dt.turning_points,"Dimension median turn spacing":dt.median_turn_spacing_months,"Latest-36m dimension turns":dt.latest_36m_dimension_turns,"Median cancellation":ca.median_cancellation,"P90 cancellation":ca.p90_cancellation,"P99 cancellation":ca.p99_cancellation,"Parent/child movement ratio":ca.parent_child_movement_ratio,"Decision":"pending"})
    final_matrix=pd.DataFrame(final_matrix)
    status=pd.DataFrame({"Policy":list(policies),"recommendation_state":"none","promotion_state":"none","human_decision":"pending"})
...
def main() -> None:
    ...
    spread_tables = _spread_correction_evidence(
        ...
        tables["capital_markets_spread_ratio_pathology_audit"])
    ...
    mw_review=f"<section><h1>Capital Markets Metric-Weight Review</h1><p><strong>The previous 1/3 Fed Funds challenger established an upper-bound warning. This finalist experiment tests whether 10%, 20%, or 25% policy-rate exposure best balances concentration and regime stability.</strong> MW-EQUAL-FAMILY is historical/secondary evidence only and is not a finalist. No similarity criterion, composite score, rank, automatic recommendation, or winner is used.</p><h2>1. Exact finalist definitions</h2>{tables['capital_markets_metric_weight_policy_registry'].to_html(index=False,border=0)}<h2>2. Decision matrix</h2>{tables['capital_markets_metric_weight_decision_matrix'].to_html(index=False,border=0,classes='decision-matrix')}<h2>3. Concentration comparison</h2>{tables['capital_markets_metric_weight_concentration_summary'].to_html(index=False,border=0)}{tables['capital_markets_metric_weight_family_contribution_summary'].to_html(index=False,border=0)}<h2>4. Fed Funds stress comparison</h2>{tables['capital_markets_metric_weight_fedfunds_stress'].to_html(index=False,border=0)}<h2>5. Extreme-jump attribution</h2>{tables['capital_markets_metric_weight_extreme_jumps'].to_html(index=False,border=0)}<h2>6. Dimension stability</h2>{tables['capital_markets_metric_weight_dimension_stability'].to_html(index=False,border=0)}<h2>7. Turning points</h2>{tables['capital_markets_metric_weight_dimension_turning_point_summary'].to_html(index=False,border=0)}<h2>8. Cancellation</h2>{tables['capital_markets_metric_weight_cancellation'].to_html(index=False,border=0,max_rows=100)}<h2>9. Recent 36-month chronology</h2>{tables['capital_markets_metric_weight_recent_chronology'].to_html(index=False,border=0)}<h2>10. Downstream context</h2><a href='evidence/capital_markets_metric_weight_axis_propagation.csv'>Axis propagation CSV</a> · <a href='evidence/capital_markets_metric_weight_regime_change_summary.csv'>Regime changes CSV</a></section>"
    polarity_review=f"<section><h1>Capital Markets Spread Polarity Review</h1><p><strong>Spread architecture decision: settled for future calibration.</strong> This is a settled diagnostic architecture, not production promotion.</p><ul><li>Legacy ratio direction-conflict counts are reconciled on unique metric × feature × date observations.</li><li>Cancellation summaries exclude non-comparable warmup rows.</li><li>The corrected spread architecture is settled.</li><li>Prior feature-weight evidence remains superseded.</li><li>Feature-weight calibration must be rerun next.</li></ul><h2>1. Exact raw spread formulas</h2>{tables['capital_markets_spread_formula_audit'].to_html(index=False,border=0)}<h2>2. Standardized sign convention</h2><p>Longer-dated yield minus shorter-dated or policy rate. Positive is upward-sloping; zero is flat; negative is inverted.</p><h2>3–4. Raw spread and sign-state chronology (zero is the economic reference line)</h2><a href='evidence/capital_markets_spread_sign_chronology.csv'>CSV</a><h2>5. Ratio-pathology examples</h2><a href='evidence/capital_markets_spread_ratio_pathology_audit.csv'>CSV</a><h2>6. Arithmetic-difference feature chronology</h2><a href='evidence/capital_markets_transform_feature_chronology.csv'>CSV</a><h2>7. Metric polarity audit</h2>{tables['capital_markets_metric_polarity_audit'].to_html(index=False,border=0)}<h2>8–9. Corrected dimension chronology and synthetic tests</h2>{tables['capital_markets_dimension_polarity_audit'].to_html(index=False,border=0)}<h2>10. Axis polarity audit</h2>{tables['capital_markets_axis_polarity_audit'].to_html(index=False,border=0)}<h2>11–12. Corrected-vs-prior stability, turning points, and decision evidence</h2><ul><li><a href='evidence/capital_markets_spread_pathology_reconciliation.csv'>Pathology reconciliation CSV</a></li><li><a href='evidence/capital_markets_spread_correction_cancellation_audit.csv'>Cancellation audit CSV</a></li><li><a href='evidence/capital_markets_spread_correction_decision_matrix.csv'>Decision matrix CSV</a></li><li><a href='evidence/capital_markets_spread_correction_dimension_chronology.csv'>Dimension chronology CSV</a></li><li><a href='evidence/capital_markets_spread_correction_dimension_stability.csv'>Dimension stability CSV</a></li><li><a href='evidence/capital_markets_spread_correction_turning_point_summary.csv'>Turning-point summary CSV</a></li><li><a href='evidence/capital_markets_spread_correction_cancellation.csv'>Cancellation CSV</a></li></ul><h2>Historical/secondary A/B/C evidence</h2><p><a href='evidence/capital_markets_combined_policy_decision_matrix.csv'>Historical A/B/C combined-policy matrix CSV</a></p><h2>13. Governance</h2><p>prior_feature_weight_evidence_status = superseded_for_final_calibration. No feature weight is selected here. Next sequence: spread polarity correction → rerun final feature-weight diagnostic → metric-weight diagnostic → Capital Markets freeze.</p></section>"
    (output/"index.html").write_text(f"<!doctype html><html><head><meta charset='utf-8'><title>Capital Markets metric-weight diagnostic</title><style>{css}</style></head><body><div class='hero'><h1>Capital Markets diagnostic review</h1><p>Diagnostic only. No automatic winner is selected; every human decision is pending.</p></div>{mw_review}{polarity_review}{final_review}<h2>Prior feature-weight evidence (superseded)</h2>{feature_review}<h2>Combined policy definitions</h2>{combined_registry_html}<h2>Combined full-history chronology</h2>{(figures/'capital_markets_combined_full_history.svg').read_text()}<h2>Combined recent chronology</h2>{(figures/'capital_markets_combined_recent.svg').read_text()}<h2>Links to six metric pages</h2><ul>{metric_links}</ul><h2>Secondary engineering evidence</h2><ul>{secondary_links}</ul><p>recommendation_state: none; promotion_state: none; human_decision: pending</p></body></html>\n",encoding="utf-8",newline="\n")
    ...
    manifest={**human_status(),"prior_feature_weight_evidence_status":PRIOR_FEATURE_WEIGHT_EVIDENCE_STATUS,"next_valid_feature_weight_experiment_must_use_settled_capital_markets_architecture":NEXT_VALID_FEATURE_WEIGHT_EXPERIMENT_MUST_USE_SETTLED_CAPITAL_MARKETS_ARCHITECTURE,"settled_diagnostic_architecture":True,"source_run_id":proof.run_id,"experiment_id":proof.experiment_id,"contract_identity":CONTRACT_IDENTITY,"native_geography_count":1,"aligned_review_geography_count":7,"one_metric_challenger_count":48,"combined_challenger_count":3,"combined_policy_registry_row_count":24,"future_metric_weight_hypothesis_only":True,"future_affordability_dependency_hypothesis_only":True,"secondary_control_count":12,"cache_build_count":24,"metric_review_page_count":6,"files":entries}
