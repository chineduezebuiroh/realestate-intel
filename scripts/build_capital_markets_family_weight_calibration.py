...
def main():
    ...
    started=perf_counter(); artifacts=load_run(args.input); load_elapsed=perf_counter()-started
    tables=build(artifacts,Path("."),load_elapsed_seconds=load_elapsed); write_review(tables,args.output)
