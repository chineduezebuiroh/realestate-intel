...
def main() -> int:
    ...
    demand_dimensions = pd.DataFrame([
        {"geo_id":"g", "date":date, "dimension":"demand", "dimension_score":.4},
        {"geo_id":"g", "date":date, "dimension":"capital_markets", "dimension_score":-.2},
    ])
    demand_parent = pd.DataFrame([{"geo_id":"g", "date":date, "axis":"demand",
                                  "axis_score": (.4*.65 + -.2*.10) / .75}])
    demand_rows, demand_rec = build_dimension_to_axis(demand_dimensions, demand_parent)
    assert set(demand_rows.dimension).issuperset({"demand", "capital_markets"})
    assert demand_rec.iloc[0].reconciliation_status == "reconciled"
    ...
    error(lambda: _coordinate_reconciliation(both_axes.query("axis == 'supply'"), coordinates), "missing")
    error(lambda: _coordinate_reconciliation(both_axes.query("axis == 'demand'"), coordinates), "missing")
    ...
    assert evidence.campaign.primary_decomposition_axes == ("supply",)
    assert evidence.campaign.supporting_coordinate_axes == ("supply", "demand")
    demand_campaign = replace(evidence.campaign, target_metric="price", target_dimension="price",
                              target_axis="demand", primary_decomposition_axes=("demand",),
                              supporting_coordinate_axes=("supply", "demand"))
    assert demand_campaign.primary_decomposition_axes == ("demand",)
    error(lambda: replace(evidence.campaign, primary_decomposition_axes=("demand",),
                          supporting_coordinate_axes=("supply",)), "subset")
    error(lambda: replace(evidence.campaign, primary_decomposition_axes=("unknown",),
                          supporting_coordinate_axes=("unknown", "supply", "demand")), "unknown axes")
    error(lambda: replace(evidence.campaign, primary_decomposition_axes=("supply", "supply")), "duplicates")
    validate_engine_decomposition(contract)
    assert set(contract.tables["dimension_to_axis"].axis) == {"supply"}
    assert set(contract.tables["axis_scope_lineage"].query("axis == 'demand'").axis_scope_role) == {"supporting_coordinate_axis"}
    out_of_scope = replace(copy.deepcopy(contract), approved_geo_ids=("different",))
    error(lambda: validate_engine_decomposition(out_of_scope), "out-of-scope geography")
    metro = copy.deepcopy(contract); metro.tables["dimension_to_axis"].loc[:, "geo_id"] = "synthetic-metro-id"; metro = replace(metro, approved_geo_ids=("synthetic-metro-id",))
    validate_engine_decomposition(metro)
