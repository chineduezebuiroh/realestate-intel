...
import numpy as np
import pandas as pd
from regime.diagnostics.affordability_feature_weight_calibration import POLICIES,ADJACENT,EXPORTS,build,load_run,write_review,_turn_evidence
...
def main():
 ...
 assert list(POLICIES)==[f"P{i}" for i in range(5)]; assert POLICIES=={"P0":(.5,.2,.3),"P1":(.45,.2,.35),"P2":(.4,.2,.4),"P3":(.4,.15,.45),"P4":(.35,.2,.45)}
 assert all(sum(x)==1 for x in POLICIES.values()); assert ("P2","P3") in ADJACENT and ("P2","P4") in ADJACENT and ("P3","P4") in ADJACENT
 tables=build(fixture(),Path(".")); assert set(EXPORTS).issubset(tables); reg=tables["scenario_registry"]; assert set(reg.scenario_id)=={f"MA12__P{i}" for i in range(5)} and set(reg.ma_window)=={"MA12_FIXED"}
 ...
 assert set(rc.reference_type)=={"raw_cycle_reference"} and set(lr.reference_type)=={"level_feature_reference","short_feature_reference","long_feature_reference"}
 assert rc.correlation.notna().any() and rc.direction_agreement.notna().any()
 boundary=rc.query("policy in ['P3','P4'] and period=='full_history'").sort_values(["metric","geo_id","policy"])
 assert set(boundary.policy)=={"P3","P4"} and boundary.correlation.notna().all()
 ...
 print("Smoke 134 passed: five policies, MA12 isolation, renormalization, propagation, comparisons, SVGs, governance, fail closed, warning cleanup")
