...
import numpy as np
import pandas as pd
from regime.diagnostics.price_feature_weight_calibration import POLICIES,ADJACENT,EXPORTS,build,load_run,write_review
 ...
 for j,geo in enumerate(REVIEW_GEOS):
  for i,date in enumerate(dates):
   ...
   for mi,m in enumerate(TARGET_METRICS):
    raw=(350000 if m=="median_sale_price" else 300)*(1+.003*i)+j
...
def main():
 ...
 with tempfile.TemporaryDirectory() as tmp:
  out=Path(tmp); write_review(tables,out); assert all((out/f"price_phase2_{x}.csv").is_file() for x in EXPORTS); svgs=list(out.glob("*.svg")); assert svgs and all("<path" in x.read_text() for x in svgs)
