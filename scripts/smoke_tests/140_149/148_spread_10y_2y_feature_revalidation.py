...
def fixture():
    ...
    weighted=pd.concat([.6*features["level"],.2*features["short"],.2*features["long"]],axis=1)
    available=pd.concat([features["level"].notna()*.6,features["short"].notna()*.2,features["long"].notna()*.2],axis=1).sum(axis=1)
    p0=weighted.sum(axis=1,min_count=1)/available.replace(0,np.nan)
    metric=pd.DataFrame({"geo_id":NATIVE_GEO,"date":dates,"metric":METRIC,"metric_score":p0})
    ...
    aligned=[]; dims=[]; axes=[]
    for geo in geos:
        aligned.extend({"geo_id":geo,"date":d,"metric_date":d,"metric":METRIC,"metric_score":p0.iloc[i]} for i,d in enumerate(dates))
    ...
    return {"normalized_features":pd.DataFrame(norm),"metric_scores":pd.concat([metric,pd.DataFrame({"geo_id":NATIVE_GEO,"date":dates,"metric":"spread_10y_fedfunds","metric_score":p0*.75})]),"source_metrics":source,"aligned_metric_scores":pd.DataFrame(aligned),"dimension_scores":pd.DataFrame(dims),"axis_scores":pd.DataFrame(axes)}

def main():
    assert METRIC=="spread_10y_2y" and list(POLICIES)==[f"P{i}" for i in range(10)] and "P10" not in POLICIES
    assert POLICIES["P0"]==(.6,.2,.2) and POLICIES["P7"]==(.35,.10,.55)
    ...
    gov=tables["governance_status"].iloc[0]; assert gov.family_metric_weight_calibration=="invalidated_pending_rerun" and not gov.automated_winner
