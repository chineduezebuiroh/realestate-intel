...
DECOMPOSITION_SECTIONS = (
    ...
    "coordinate_reconciliation", "regime_reconciliation", "axis_scope_lineage",
)
...
def _coverage_tables(artifacts: Mapping[str, pd.DataFrame], f2m: pd.DataFrame, r1: pd.DataFrame,
                     m2d: pd.DataFrame, r2: pd.DataFrame, d2a: pd.DataFrame, r3: pd.DataFrame,
                     primary_axes: tuple[str, ...]) -> pd.DataFrame:
    rows = []
    ...
    for geo in geos:
        ...
        for parent_key in primary_axes:
...
def _coordinate_reconciliation(axes: pd.DataFrame, coordinates: pd.DataFrame) -> pd.DataFrame:
    ...
    missing_axes = {"supply", "demand"}.difference(pivot.columns)
    if missing_axes:
        raise ValueError(f"Supporting coordinate axes are missing: {sorted(missing_axes)}")
...
def build_engine_decomposition(*, campaign, series_artifacts: Mapping[str, Mapping[str, pd.DataFrame]], selected_geographies: list[str]) -> EngineDecompositionEvidence:
    ...
    approved = tuple(sorted(set(map(str, selected_geographies))))
    if not approved or len(approved) != len(selected_geographies):
        raise ValueError("Approved decomposition geography universe must be nonempty and unique")
    primary_axes = tuple(campaign.primary_decomposition_axes)
    supporting_axes = tuple(campaign.supporting_coordinate_axes)
    ...
    for series_id, artifacts in series_artifacts.items():
        ...
        scoped = {name: frame[frame["geo_id"].astype(str).isin(approved)].copy()
                  for name, frame in artifacts.items() if isinstance(frame, pd.DataFrame) and "geo_id" in frame}
        observed = set().union(*(set(frame["geo_id"].astype(str)) for frame in scoped.values()))
        if not observed.issubset(set(approved)):
            raise ValueError("Decomposition contains a geography outside the approved universe")
        f2m, r1 = build_feature_to_metric(scoped["normalized_features"], scoped["metric_scores"])
        m2d, r2 = build_metric_to_dimension(scoped["aligned_metric_scores"], scoped["dimension_scores"])
        primary_axis_scores = scoped["axis_scores"][scoped["axis_scores"]["axis"].isin(primary_axes)]
        d2a, r3 = build_dimension_to_axis(scoped["dimension_scores"], primary_axis_scores)
        supporting_axis_scores = scoped["axis_scores"][scoped["axis_scores"]["axis"].isin(supporting_axes)]
        cr = _coordinate_reconciliation(supporting_axis_scores, scoped["coordinates"])
        rr = _regime_reconciliation(scoped["coordinates"], scoped["regime_assignments"])
        ...
        coverage = _coverage_tables(scoped, f2m, r1, m2d, r2, d2a, r3, primary_axes)
        collected["chronology_coverage"].append(_add_identity(coverage[coverage["geo_id"].isin(selected_geographies)], campaign, series_id))
    registry_axes = tuple(_build_axis_weights()["axis"].drop_duplicates())
    lineage = []
    for axis in registry_axes:
        is_primary, is_supporting = axis in primary_axes, axis in supporting_axes
        role = "primary_decomposition_axis" if is_primary else ("supporting_coordinate_axis" if is_supporting else "out_of_scope_axis")
        reason = "strict_campaign_axis_decomposition" if is_primary else ("supporting_axis_not_decomposed_in_this_campaign" if is_supporting else "axis_not_used_by_campaign")
        lineage.append({"campaign_id": campaign.campaign_id, "campaign_version": campaign.campaign_version,
                        "axis": axis, "axis_scope_role": role, "strict_decomposition_required": is_primary,
                        "supporting_coordinate_required": is_supporting, "scope_reason": reason,
                        "authoritative_registry_present": True})
    collected["axis_scope_lineage"] = [pd.DataFrame(lineage)]
    ...
    evidence = EngineDecompositionEvidence(campaign.campaign_id, campaign.campaign_version,
        campaign.candidate_policy_ids, tables, primary_axes, supporting_axes, approved)
...
def validate_engine_decomposition(evidence: EngineDecompositionEvidence) -> None:
    ...
    for name in DECOMPOSITION_SECTIONS:
        ...
        if name != "axis_scope_lineage" and set(frame["series_id"].astype(str)) != series: raise ValueError(f"{name} candidate identity mismatch")
    ...
    lineage = evidence.tables["axis_scope_lineage"]
    roles = {"primary_decomposition_axis", "supporting_coordinate_axis", "out_of_scope_axis"}
    if not set(lineage["axis_scope_role"]).issubset(roles):
        raise ValueError("Axis scope lineage contains an unknown role")
    registered_axes = set(_build_axis_weights()["axis"])
    if not set(lineage["axis"]).issubset(registered_axes) or not lineage["authoritative_registry_present"].astype(bool).all():
        raise ValueError("Axis scope lineage contains an unregistered axis")
    if set(evidence.primary_decomposition_axes).difference(evidence.supporting_coordinate_axes):
        raise ValueError("Primary decomposition axes must be supporting coordinate axes")
    primary_rows = lineage[lineage["strict_decomposition_required"].astype(bool)]
    if set(primary_rows["axis"]) != set(evidence.primary_decomposition_axes):
        raise ValueError("Axis scope lineage primary identity mismatch")
    supporting_rows = lineage[lineage["supporting_coordinate_required"].astype(bool)]
    if set(supporting_rows["axis"]) != set(evidence.supporting_coordinate_axes):
        raise ValueError("Axis scope lineage supporting identity mismatch")
    axis_rows = evidence.tables["dimension_to_axis"]
    if set(axis_rows["axis"]) != set(evidence.primary_decomposition_axes):
        raise ValueError("dimension_to_axis must contain exactly the primary decomposition axes")
    for series_id, part in axis_rows.groupby("series_id"):
        if set(part["axis"]) != set(evidence.primary_decomposition_axes):
            raise ValueError(f"dimension_to_axis is missing a primary axis for {series_id}")
    if evidence.approved_geo_ids and not set(axis_rows["geo_id"].astype(str)).issubset(set(evidence.approved_geo_ids)):
        raise ValueError("dimension_to_axis contains an out-of-scope geography")
