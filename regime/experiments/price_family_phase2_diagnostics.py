...
TURN_PROMINENCE_MIN_SIDE_OBSERVATIONS = 3
TURN_PROMINENCE_MIN_SIDE_COVERAGE_MONTHS = 5
...
def _calendar_prominence_windows(
    work: pd.DataFrame,
    candidate_index: int,
    *,
    review_col: str,
    ...
    candidate_date = pd.Timestamp(work.loc[candidate_index, "date"])
    prior_start = candidate_date - pd.DateOffset(months=window_months)
    following_end = candidate_date + pd.DateOffset(months=window_months)
    ...
    if len(prior) < TURN_PROMINENCE_MIN_SIDE_OBSERVATIONS:
        return None, None
    if len(following) < TURN_PROMINENCE_MIN_SIDE_OBSERVATIONS:
        return None, None
    required_prior_start = candidate_date - pd.DateOffset(
        months=TURN_PROMINENCE_MIN_SIDE_COVERAGE_MONTHS
    )
    required_following_end = candidate_date + pd.DateOffset(
        months=TURN_PROMINENCE_MIN_SIDE_COVERAGE_MONTHS
    )
    if pd.to_datetime(prior["date"]).min() > required_prior_start:
        return None, None
    if pd.to_datetime(following["date"]).max() < required_following_end:
        return None, None
    return prior[review_col], following[review_col]
    ...
    for index in range(1, len(work) - 1):
        ...
        if value >= prev_value and value > next_value:
            ...
            if prominence_window_months is None:
                ...
                before, after = _calendar_prominence_windows(
                    work,
                    index,
                    review_col=review_col,
                    window_months=prominence_window_months,
                )
...
def _turning_lag(
                ...
                row.update(
                    {
                        ...
                        "turn_prominence_min_side_observations": TURN_PROMINENCE_MIN_SIDE_OBSERVATIONS if use_smoother else np.nan,
                        "turn_prominence_min_side_coverage_months": TURN_PROMINENCE_MIN_SIDE_COVERAGE_MONTHS if use_smoother else np.nan,
    ...
    columns = [
        ...
        "turn_prominence_window_months", "turn_prominence_min_side_observations",
        "turn_prominence_min_side_coverage_months", "turn_max_match_window_months",
    ]
