"""Bounded diagnostic-only LAUS MA x long-weighted feature architecture calibration.
...
REQUIRED_EXPORTS = (
    ...
    "laus_long_weight_controlled_comparisons", "laus_long_weight_family_response_curves",
    "laus_long_weight_interactions", "laus_long_weight_by_county",
    "laus_long_weight_cross_metric_consistency", "laus_long_weight_evaluation_matrix",
    "laus_long_weight_governance_status",
)
VISUAL_FAMILIES = (
...
def scenario_registry() -> pd.DataFrame:
    ...
    out=pd.DataFrame(rows); expected={f"MA{m}__{p}" for m in MA_WINDOWS for p in FEATURE_WEIGHTS}
    if len(out)!=22 or set(out.scenario_id)!=expected: raise AssertionError("exactly 22 factor scenarios required")
...
def feature_anatomy(metric: pd.DataFrame) -> pd.DataFrame:
    """Describe normalized features before configured weights are applied."""
    base=metric.loc[metric.weight_policy.eq("W0")].copy()
    rows=[]
    for period,q in _periods(base):
        for (ma,geo,name),g in q.groupby(["ma_months","geo_id","metric"]):
            correlations=g[[f+"_score" for f in FEATURES]].corr()
            for feature in FEATURES:
                value=feature+"_score"; row={"period":period,"ma_months":ma,
                    "geo_id":geo,"metric":name,"feature":feature,**_stats(g,value),
                    "correlation_with_raw_laus":g[value].corr(g.raw_value) if "raw_value" in g else np.nan}
                for other in FEATURES:
                    row[f"correlation_with_{other}"]=correlations.loc[value,other+"_score"]
                rows.append(row)
    return pd.DataFrame(rows)
...
def _comparisons(frame: pd.DataFrame,value: str,registry: pd.DataFrame):
    ...
    for policy in FEATURE_WEIGHTS:
        ...
        ma.append(row)
    response=[]
    pairs=(("W0","B0"),("B0","B1"),("B0","B2"),("B0","B3"),
           ("B2","B3"),("B3","L1"),("L0","L1"),("L1","L2"),
           ("L2","L3"),("W3","B3"),("W5","B3"),("W5","L1"),("W5","L2"))
    for horizon in MA_WINDOWS:
        for left,right in pairs:
                a_id=f"MA{horizon}__{left}"; b_id=f"MA{horizon}__{right}"
                sa=full.loc[full.scenario_id.eq(a_id)].iloc[0]; sb=full.loc[full.scenario_id.eq(b_id)].iloc[0]
                row={"ma_months":horizon,"comparison_type":"critical_pair","from_scenario":a_id,"to_scenario":b_id,
...
def _plots(output: Path,source: pd.DataFrame,raw_panel: pd.DataFrame,metric: pd.DataFrame,
    ...
    colors=dict(zip(FEATURE_WEIGHTS,plt.cm.viridis(np.linspace(.05,.95,len(FEATURE_WEIGHTS)))))
    ...
    anatomy=metric.loc[metric.weight_policy.eq("W0")].groupby(
        ["ma_months","metric","date"],as_index=False)[[f+"_score" for f in FEATURES]].mean()
    for (ma,name),q in anatomy.groupby(["ma_months","metric"]):
        fig,ax=plt.subplots(figsize=(10,4))
        for feature in FEATURES: ax.plot(q.date,q[feature+"_score"],label=feature.title())
        ax.set_title(f"{name} — MA{ma} unweighted normalized feature anatomy"); ax.legend(); fig.tight_layout()
        fig.savefig(visual/f"feature_anatomy__{name}__MA{ma}.svg"); plt.close(fig)
    for (name,ma),q in pooled.groupby(["metric","ma_months"]):
        fig,ax=plt.subplots(figsize=(10,4))
        for (policy,label),x in q.groupby(["weight_policy","policy_label"]): ax.plot(x.date,x.metric_score,label=label,color=colors[policy])
    ...
    rep=rep.loc[rep.weight_policy.isin(["W0","B0","B3","L1","L2","W5"])].groupby(["ma_months","weight_policy","metric","feature","date"],as_index=False).weighted_contribution.mean()
    ...
    families={"balanced":("B0","B1","B2","B3"),"long_heavy":("L0","L1","L2","L3"),
              "controls":("W0","W3","W5")}
    for family,policies in families.items():
        for value,data in (("metric_score",pooled),("cyclical_score",pool),("core_demand_score",pool)):
            for ma,q in data.loc[data.weight_policy.isin(policies)].groupby("ma_months"):
                fig,axes=plt.subplots(2,2,figsize=(12,7),sharex=True)
                for policy,ax in zip(policies,axes.flat):
                    x=q.loc[q.weight_policy.eq(policy)]
                    if value=="metric_score": x=x.groupby("date",as_index=False)[value].mean()
                    ax.plot(x.date,x[value]); ax.set_title(LABELS[policy])
                for ax in axes.flat[len(policies):]: ax.axis("off")
                fig.suptitle(f"{family} — {value} — MA{ma}"); fig.tight_layout()
                fig.savefig(visual/f"family__{family}__{value}__MA{ma}.svg"); plt.close(fig)
    for value,data in (("metric_score",pooled),("cyclical_score",pool),("core_demand_score",pool)):
        for ma,q in data.loc[data.weight_policy.isin(["B2","B3"])].groupby("ma_months"):
            fig,ax=plt.subplots(figsize=(10,4))
            grouping=["weight_policy","metric"] if value=="metric_score" else ["weight_policy"]
            for key,x in q.groupby(grouping): ax.plot(x.date,x[value],label=str(key))
            ax.set_title(f"B2 versus B3 — {value} — MA{ma}"); ax.legend(); fig.tight_layout()
            fig.savefig(visual/f"b2_vs_b3__{value}__MA{ma}.svg"); plt.close(fig)
    ...
    for measure,data in measures:
        ...
        ax.set_xticks(range(len(FEATURE_WEIGHTS)),FEATURE_WEIGHTS); ax.set_yticks(range(2),["MA6","MA9"])
        for i in range(2):
            for j in range(len(FEATURE_WEIGHTS)): ax.text(j,i,f"{table.iloc[i,j]:.2f}",ha="center",va="center",color="white")
...
def build_review(run: Path,output: Path,root: Path|None=None) -> Path:
    ...
    anatomy=feature_anatomy(metric)
    ...
    def metric_extra(g):
        ...
        candidate_direction = np.sign(candidate.diff())
        w0_direction = np.sign(w0_reference.diff())
        production_direction = np.sign(production_reference.diff())
        ...
        return {
            ...
            "direction_agreement_with_w0":
                candidate_direction.iloc[1:].eq(w0_direction.iloc[1:]).mean(),
            "direction_agreement_with_w5":
                candidate_direction.iloc[1:].eq(production_direction.iloc[1:]).mean(),
        }
    ...
    family_map={**{p:"controls" for p in ("W0","W3","W5")},
        **{p:"balanced" for p in ("B0","B1","B2","B3")},
        **{p:"long_dominant" for p in ("L0","L1","L2","L3")}}
    family_response=core_stats.loc[core_stats.period.eq("full_history")].copy()
    family_response["policy_family"]=family_response.weight_policy.map(family_map)
    cross_metric=metric_stats.loc[metric_stats.period.eq("full_history")].groupby(
        ["scenario_id","ma_months","weight_policy","metric"],as_index=False).mean(numeric_only=True)
    cross_metric["common_weighting_review_required"]=False
    ...
    exports={"laus_long_weight_scenario_registry":registry,
        ...
        "laus_long_weight_core_demand_statistics":core_stats,
        "laus_long_weight_demand_axis_statistics":axis_stats,
        "laus_long_weight_ma_effects":ma_effects,
        "laus_long_weight_controlled_comparisons":response,
        "laus_long_weight_family_response_curves":family_response,
        "laus_long_weight_interactions":interactions,
        "laus_long_weight_by_county":by_county,
        "laus_long_weight_cross_metric_consistency":cross_metric,
        "laus_long_weight_evaluation_matrix":evaluation,
        "laus_long_weight_governance_status":governance,
        "laus_long_weight_raw_ma_statistics":raw_stats}
    ...
    (output/"laus_long_weight_review.html").write_text("<!doctype html><meta charset='utf-8'><title>LAUS MA × long-weighted feature architecture review</title><h1>LAUS MA × long-weighted feature architecture review</h1><p>Diagnostic only. No recommendation, automated winner, or production change.</p><h2>Evidence</h2><ul>"+links+"</ul><h2>Visuals</h2><ul>"+visuals+"</ul>",encoding="utf-8")
    return output
