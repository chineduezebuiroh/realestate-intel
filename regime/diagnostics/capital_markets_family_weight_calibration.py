...
AUTHORITATIVE_RUN = Path("artifacts/regime/runs/capital_markets_feature_policy_corrected_production_20260818")
FAMILIES = {
...
NATIVE_POLICIES = {"mortgage_30y":"P4", "mortgage_15y":"P2", "treasury_10y":"P1", "fedfunds":"P5", "spread_10y_2y":"P6", "spread_10y_fedfunds":"P9"}
...
def build(artifacts: dict[str,pd.DataFrame], root: Path, *, load_elapsed_seconds: float = 0.) -> dict[str,pd.DataFrame]:
    started=perf_counter(); stage_started=started; timings=[]
    def timed(stage: str, architecture: str) -> None:
        nonlocal stage_started
        now=perf_counter(); timings.append({"stage":stage,"elapsed_seconds":now-stage_started,"architecture":architecture}); stage_started=now
    timings.append({"stage":"artifact load","elapsed_seconds":load_elapsed_seconds,"architecture":"corrected persisted aligned scores; no fallback or feature construction"})
    ...
    timed("candidate construction","closed F0-F9 panels and exact F0 reconstruction")
    ...
    timed("contribution calculations","metric and family anatomy, cancellation, concentration, and net-to-gross inputs")
    ...
    timed("stability statistics","period and county stability, incumbent comparison, and contribution structure")
    ...
    timed("overlap/consistency","corrected-baseline family overlap and equal-weight within-family consistency")
    ...
    timed("responsiveness","fixed invariant family-move thresholds across candidates")
    ...
    timed("downstream propagation","frozen Demand/Supply propagation and cross-axis materiality")
    ...
    questions=["Does F0 = 45/10/45 remain defensible after the spread polarity repair?","Has the realized influence of the spread family increased materially?","Does the previous apparent spread-family cancellation disappear?","Does 10% Fed Funds remain appropriate?","Does the Rates-versus-Spreads plateau shift?","Are F1/F2 or F3/F4 now more credible?","Does reducing Fed Funds to 5% remain credible?","Does increasing Fed Funds to 15-25% still add value?","Are the two corrected spread metrics internally coherent enough for equal weighting?","Does evidence justify a future intra-spread weighting experiment?","Do Demand and Supply prefer materially different family balances?","What are the corrected 2-3 credible finalists?","Where does the practical optimization plateau begin?"]
    evaluation=pd.DataFrame({"question":questions,"status":"empirical_review_required","automated_ranking":False})
    governance=pd.DataFrame([{"corrected_baseline_identity":AUTHORITATIVE_RUN.name,"recommendation_state":"none","promotion_state":"current_production_unchanged","human_decision":"capital_markets_family_weight_review_pending","automated_winner":False,"production_policy_changed":False,"feature_weight_policy_changed":False,"metric_weight_policy_changed":False,"Demand_changed":False,"Supply_changed":False,"Capital_Markets_changed":False,"native_feature_calibration":"closed","spread_polarity_repair":"closed","family_metric_weight_calibration":"in_review","candidate_grid_closed":True,"candidate_grid":"F0-F9","intra_family_metric_weight_calibration":"not_started","prior_invalidated_evidence_reused":False,"supply_s8_changed":False}])
    timings.append({"stage":"total analytical build","elapsed_seconds":perf_counter()-started,"architecture":"diagnostic wall clock excluding artifact load and review rendering"})
    performance=pd.DataFrame(timings)
...
def write_review(tables: dict[str,pd.DataFrame], out: Path) -> None:
    out.mkdir(parents=True,exist_ok=True); prefix="capital_markets_family_weight"; visual_started=perf_counter()
    ...
    tables["performance_audit"] = pd.concat([tables["performance_audit"],pd.DataFrame([{"stage":"visualization","elapsed_seconds":perf_counter()-visual_started,"architecture":"deterministic SVG review package"}])],ignore_index=True)
    export_started=perf_counter()
    for name in EXPORTS:
        if name != "performance_audit": tables[name].to_csv(out/f"{prefix}_{name}.csv",index=False)
    ...
    tables["performance_audit"] = pd.concat([tables["performance_audit"],pd.DataFrame([{"stage":"export/write","elapsed_seconds":perf_counter()-export_started,"architecture":"CSV exports and self-contained HTML index"}])],ignore_index=True)
    tables["performance_audit"].to_csv(out/f"{prefix}_performance_audit.csv",index=False)
