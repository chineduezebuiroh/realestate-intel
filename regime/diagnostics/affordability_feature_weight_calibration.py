"""Affordability Phase-2 bounded feature-weight calibration.

This diagnostic only reweights persisted normalized MA12 Affordability features.  It
...
from regime.diagnostics.affordability_feature_anatomy import load_run as _load_phase1
...
def resolve_contract(root: Path):
    return canonical.resolve_contract(root, "affordability")
...
def build_raw_cycle(source: pd.DataFrame, contract: pd.DataFrame) -> pd.DataFrame:
    """Build a diagnostic raw-affordability cycle with exact calendar lag-12 semantics.
    ...
    if raw.empty:
        raise ValueError(
            "No authoritative raw Affordability chronology resolved "
    ...
    if missing_metrics:
        raise ValueError(
            "Authoritative raw Affordability chronology is missing "
            f"canonical metrics={sorted(missing_metrics)}"
        )
    if raw.duplicated(["geo_id","date","metric"]).any():
        raise ValueError("duplicate raw Affordability source observation")
    panels=[]
...
def build(artifacts: dict[str,pd.DataFrame], root: Path) -> dict[str,pd.DataFrame]:
    ...
    if set(contract.window_lag_definition.astype(str)) != {"12m", "12m/lag3m", "12m/lag12m"}:
        raise ValueError("Affordability Phase 2 requires the governed MA12/lag3/lag12 feature family")
    ...
    if base.duplicated(["geo_id","date","metric","feature_type"]).any(): raise ValueError("duplicate persisted normalized Affordability feature")
    panels=[]
    ...
    wide["affordability_dimension_score"] = values.mul(metric_weights).sum(axis=1, min_count=1).div(denom)
    dim=[]
    for (p,g),z in wide.groupby(["policy","geo_id"]):
      for period,q in _periods(z):
       gross=q[list(TARGET_METRICS)].abs().mul(metric_weights).sum(axis=1).div(q[list(TARGET_METRICS)].notna().mul(metric_weights).sum(axis=1)); row={"policy":p,"geo_id":g,"period":period,**_extra_stats(q.affordability_dimension_score,q.date)}
       row["metric_level_cancellation"]=(1-q.affordability_dimension_score.abs().div(gross.replace(0,np.nan))).mean(); dim.append(row)
    dim=pd.DataFrame(dim)
    # Propagate the unchanged registry-resolved Affordability delta through persisted P0 Demand.
    ...
    p0=wide[wide.policy.eq("P0")][["geo_id","date","affordability_dimension_score"]].rename(columns={"affordability_dimension_score":"p0_affordability"})
    axis_registry = pd.read_csv(root / "config/axis_registry.csv")
    demand_aff = axis_registry[(axis_registry.axis.eq("demand")) & (axis_registry.dimension.eq("affordability")) & axis_registry.enabled.astype(str).str.lower().isin(("true", "1", "yes"))]
    if len(demand_aff) != 1: raise ValueError("Demand/Affordability axis weight is missing or ambiguous")
    demand_weight = float(demand_aff.dimension_weight.iloc[0])
    dw=wide.merge(p0,on=["geo_id","date"],validate="many_to_one").merge(demand,on=["geo_id","date"],validate="many_to_one")
    dw["demand_axis_score"]=dw.p0_demand_axis+demand_weight*(dw.affordability_dimension_score-dw.p0_affordability)
    dst=[]
    for (p,g),z in dw.groupby(["policy","geo_id"]):
      for period,q in _periods(z):
       row={"policy":p,"geo_id":g,"period":period,**_extra_stats(q.demand_axis_score,q.date)}
       p0_stats=_extra_stats(q.p0_demand_axis,q.date)
       row["chronology_correlation_to_p0"]=q.demand_axis_score.corr(q.p0_demand_axis)
...
def write_review(tables, out: Path):
    ...
    for name in EXPORTS: tables[name].to_csv(out/f"affordability_phase2_{name}.csv",index=False)
    ...
    for metric in TARGET_METRICS:
      ...
      for policy in ("P0", "P2", "P3", "P4", "P5"):
       ...
       fn=f"affordability_phase2_{metric}_{policy}_contribution_decomposition.svg"
    ...
    for subject in (*TARGET_METRICS,"affordability_dimension"):
      frame=tables["metric_statistics"] if subject!="affordability_dimension" else tables["dimension_statistics"]
      q=frame[frame.period.eq("full_history")]
      if subject!="affordability_dimension": q=q[(q.metric.eq(subject))&q.geo_id.eq("seven_county_mean")]
      ...
      fn=f"affordability_phase2_{subject}_response_curves.svg"; _svg(out/fn,series,f"{subject} response curves"); plots.append(fn)
    files=[*(f"affordability_phase2_{n}.csv" for n in EXPORTS),*plots]
    links="".join(f'<li><a href="{html.escape(x)}">{html.escape(x)}</a></li>' for x in files)
    (out/"affordability_phase2_review_index.html").write_text(f"<!doctype html><meta charset=utf-8><title>Affordability Phase 2</title><h1>Affordability feature-weight calibration</h1><p>Diagnostic only; human review pending; production unchanged; MA12 fixed.</p><ul>{links}</ul>",encoding="utf-8")
