"""Durable LAUS candidate publication and exact accepted-prior resolution."""
...
def accepted_uri(catalog: dict[str, Any]) -> str | None:
    ...
    if len(matches) != 1:
        raise RuntimeError("accepted LAUS pointer does not identify exactly one immutable record")
...
def resolve_prior(*, api: GitHubAPI, cas: GitHubCatalogCAS, workspace: Path,
    ...
    """Resolve only the durable accepted LAUS state; no legacy fallback exists."""
    catalog, _ = cas.read()
    ...
    raise RuntimeError("accepted LAUS durable prior is required")
...
def _published_candidate(catalog: dict[str, Any], manifest: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if len(matches) > 1:
        raise RuntimeError("catalog contains duplicate LAUS candidate identity")
    if not matches:
        return None
    record = matches[0]
    if record["artifact_content_hash"] != manifest["artifact_content_hash"]:
        raise RuntimeError("cataloged LAUS candidate content identity conflicts")
    return record
...
def resolve_published_candidate(*, artifact: Path, api: GitHubAPI, cas: GitHubCatalogCAS,
    ...
    if any(local.get(field) != remote.get(field) for field in immutable_fields):
        raise RuntimeError("cataloged LAUS candidate differs in immutable artifact evidence")
...
def publish(*, artifact: Path, api: GitHubAPI, cas: GitHubCatalogCAS, workspace: Path,
    ...
    if resolved_manifest["data_sha256"] != manifest["data_sha256"]:
        raise RuntimeError("durable resolver did not reproduce LAUS data identity")
    ...
    if args.command == "resolve-prior":
        ...
        try:
            ...
            write_canonical_json(args.output.with_name("publication_failure.json"), {
                ...
                "schema_version": "laus_publication_failure_v1",
            })
            raise
