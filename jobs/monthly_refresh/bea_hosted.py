"""Common durable-pin execution path for independent BEA physical sources."""
...
from jobs.monthly_refresh.bea_monthly import (MEMBER, candidate, discover_pin,
...
def execute_source(*, source_id: str, mode: str, cycle_id: str, workspace: Path,
                   pin_store: Any, discover: Callable[[], tuple[dict, dict[str, Path], dict]],
    ...
    cached: dict[str, Path] = {}; lineage: dict[str, Any] = {}
    def discovery() -> dict:
        pin, paths, acquired = discover(); cached.update(paths); lineage.update(acquired); return pin
    def execution(pin: Mapping[str, Any]) -> dict[str, Any]:
        paths = cached
        if not paths:
            path = workspace / "pinned-input" / "snapshot.json"
...
def main() -> int:
    ...
    value = execute_source(source_id=args.source_id, mode=args.mode, cycle_id=args.cycle_id,
