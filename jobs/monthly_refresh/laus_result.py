"""Fail-closed construction of the LAUS monthly source execution result."""
...
def build_result(*, root: Path, cycle_id: str, acquire_outcome: str,
                 publish_outcome: str, acquisition_mode: str) -> dict[str, Any]:
    ...
    result = {"schema_version": "monthly_source_execution_result_v1", "source_id": "laus",
        ...
        "accepted_pointer_changed": False, "evidence_uri": "actions://laus/run_report.json"}
    ...
    required_run = ("acquisition_mode", "resulting_artifact_id", "resulting_artifact_content_hash", "observation_max",
    ...
    if not run or run.get("run_status") not in {"refreshed", "unchanged"} or not manifest \
            ...
            or run.get("acquisition_mode") != acquisition_mode:
        return result
    ...
    if not publication or not publication.get("package_sha256") \
            ...
            or publication.get("accepted_pointer_changed") is not False \
            or publication.get("durable_resolution_passed") is not True:
        return result
    result.update(status="succeeded", package_sha256=publication["package_sha256"],
        ...
        evidence_uri="artifact://source/laus/" + run["resulting_artifact_id"])
    return result
