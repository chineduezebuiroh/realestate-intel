...
The contract version is `engine_decomposition_v5`; it is recorded in both the
...
Campaigns declare ordered `primary_decomposition_axes` and
`supporting_coordinate_axes`. The Inventory campaign declares Supply as its
primary strict-decomposition axis and Supply plus Demand as supporting geometry
axes. Dimension → Axis evidence is limited to primary axes and the already
resolved governed geography universe; supporting axes remain authoritative
persisted inputs to coordinates and regimes. Axis-scope lineage records each
registry axis as primary, supporting-only, or out of scope. Capital Markets is
never removed when it is configured as a contributor to a primary axis.

Historical persisted Demand-axis scores do not reconcile to a rebuild using the
current dimension configuration for many county and CBSA rows. The likely cause
is a producing-configuration/provenance mismatch until explicitly resolved.
Demand remains a required persisted supporting input to `y_demand` and regime
geometry, but its contribution decomposition is not claimed valid by this
Supply-focused campaign. Strict Demand decomposition is deferred until a
Demand-focused campaign or provenance reconciliation; a future Demand-primary
campaign must fail closed under its authoritative producing configuration.
