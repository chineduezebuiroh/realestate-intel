...
from forecast.core.db_forecast import new_batch_id, record_xgb_selector_run, get_connection
...
def run_xgb_selector(
    ...
    *,
    ...
    """
    XGB SELECTOR (artifacts + forecast_runs audit row).
    ...
    Hard contracts:
      ...
      - writes forecast_runs row for auditability (no predictions table writes)
    ...
    if Path(artifact_root).name == "runs":
        raise ValueError("[xgb_selector] artifact_root should be the phase root (e.g. artifacts/phasec), not .../runs")
    ...
    try:
        ...
        # compute paths (you already do this)
        out_path = out_dir / f"selected_features__anchor={anchor_date.date().isoformat()}.parquet"
        cand_out = out_dir / f"candidate_scores__anchor={anchor_date.date().isoformat()}.parquet"
        out_json = out_dir / f"selector_summary__anchor={anchor_date.date().isoformat()}.json"

        # overwrite guards BEFORE DB transaction
        for p, label in [(out_path, "artifact"), (cand_out, "candidate_scores"), (out_json, "summary")]:
            if p.exists():
                raise SystemExit(
                    f"[xgb_selector] REFUSING to overwrite existing {label}: {p}\n"
        ...
        # 9a) prepare candidate scores artifact from STAGE-1 scored candidates (base-series)
        # NOTE: FeatureSpec.name is base id (no _lagN). XGB selected_features are lagged.
        # We mark selected at base-series level by stripping _lagN from fi_sel ids.
        def _strip_lag(fid: str) -> str:
            # feature ids look like "..._lag1", "..._lag12"
            # safe even if no lag suffix present
            import re
            return re.sub(r"_lag\d+$", "", str(fid))

        selected_base_ids = {_strip_lag(x) for x in fi_sel["feature_id"].astype(str).tolist()}

        rows = []
        for r in scored:
            base_id = str(r.spec.name)  # base-series identity per FeatureSpec contract
            extras = dict(r.extras or {})

            rows.append(
                {
                    "feature_id": base_id,                 # base-series id (NO lag suffix)
                    "stage1_score": float(r.score),
                    "stage1_rank": None,                   # set after sort
                    "best_lead": int(r.best_lead),
                    "n_eff": int(r.n_eff),
                    "selected": base_id in selected_base_ids,

                    # lift diagnostics (only present for cheap_lift)
                    "lift_vs_baseline": extras.get("lift_vs_baseline"),
                    "mae_model": extras.get("mae_model"),
                    "mae_baseline": extras.get("mae_baseline"),
                    "val_months_used": extras.get("val_months_used"),
                    "horizon_months": extras.get("horizon_months"),
                    "score_mode": extras.get("score_mode"),
                }
            )

        cand = pd.DataFrame(rows)
        if cand.empty:
            raise SystemExit("[xgb_selector] FAIL: empty candidate_scores (no scored candidates).")
        ...
        cand["geo_id"] = geo_id
        cand["property_type_id"] = property_type_id
        cand["anchor_date"] = anchor_date.date().isoformat()
        cand["seed"] = int(seed)
        cand["data_asof_requested"] = data_asof_requested.isoformat() if data_asof_requested else None
        ...
        if cand["feature_id"].duplicated().any():
            raise ValueError("[xgb_selector] candidate_scores has duplicate base feature_id rows; investigate scored construction")
        ...
        summary = SelectorRunSummary(
            ...
            out_candidate_parquet=str(cand_out),
        ...
        try:
            ...
            record_xgb_selector_run(
                ...
                seed=int(seed),
                stage1_mode=str(stage1_score_mode),
                run_kind="selector_backtest",
                algo_params={
                    "selector_kind": "selector_backtest",
                    "selection_intent": "backtest_eval",
