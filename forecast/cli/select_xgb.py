...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser("forecast.cli.select_xgb")
    p.add_argument("--batch_id", required=False)
    p.add_argument("--data_asof", required=False)
    p.add_argument("--anchor_date", required=False)
    return p
...
def main(argv: list[str] | None = None) -> int:
    ...
    from forecast.backtest_xgb_single import main as legacy_main
